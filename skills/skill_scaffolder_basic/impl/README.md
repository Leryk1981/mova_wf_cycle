# Implementation for `skill.skill_scaffolder_basic`

This folder contains the green-layer implementation:

- `prompts/` – prompt profile for LLM-based execution:
  - `skill_scaffolder_profile.md` – instructions for the LLM on how to generate skill scaffolds

- `bindings/` – runtime binding definitions:
  - `skill_scaffolder_llm_binding_v1.json` – LLM profile binding (no external calls)
...
This skill uses an `llm_profile` runtime type, meaning it is executed by an LLM following the prompt profile. No external HTTP requests, MCP calls, or local scripts are involved.
