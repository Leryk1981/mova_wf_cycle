...
function run() {
  ...
  const baseDir = path.join(repoRoot, "artifacts", "domain_pack_scaffold", runId);
  ...
  const policy = {
    ...
    notes: "deny-by-default policy for domain_pack_scaffold bundles"
  };
  ...
  const roleBundles = {
    ...
    roles: allowedRoles.map((role) => ({
      role_id: role,
      description: `Canonical ${role} role for domain_pack_scaffold bundles`,
      capabilities: [
        role === "planner"
          ? "plan"
          : role === "executor"
...
try {
  ...
  const code = error.code || "ERR_DOMAIN_PACK_SCAFFOLD_GENERATE";
  console.error(`[domain_pack_scaffold_generate] ${code} ${error.message}`);
  process.exit(1);
}
