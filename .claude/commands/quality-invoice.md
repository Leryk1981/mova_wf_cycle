---
description: Run Invoice/AP quality suites (pos + neg) and summarize results with artifact paths.
argument-hint: [fix]  (fix=attempt minimal fixes if a suite fails)
allowed-tools: Bash(git status:*), Bash(git diff:*), Bash(git branch --show-current), Bash(npm run quality:invoice_ap), Bash(npm run quality:invoice_ap:neg)
---
...
1) npm run quality:invoice_ap
2) npm run quality:invoice_ap:neg
